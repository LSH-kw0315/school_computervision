...
void CMFCApplication3Dlg::OnBnClickedMainImagedisplay()
{
	...
	cv::Mat clone_of_original = original_Image.clone();
	...
	if (original_Image.channels()==3&&rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) { //회색조 변환
		clone_of_original = cv::Mat(original_Image.cols, original_Image.rows, CV_8UC1);
		for (int i = 0; i < original_Image.rows;i++) {
			for (int j = 0; j < original_Image.cols; j++) {
				clone_of_original.data[i*original_Image.cols+j] = (int)(0.114 * original_Image.data[3 * (i * original_Image.cols + j)] + 0.587 * original_Image.data[3 * (i*original_Image.cols + j) + 1] + 0.299 * original_Image.data[3 * (i*original_Image.cols + j) + 2]);
			}
		}
	}
	...
	}else if(scaling_dlg.GetInterpolation() == InterpolationMode::BiCubic) {//바이큐빅
	...
	if (save_dlg.DoModal() ==IDOK ) {
		...
		if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
			...
			unsigned char* only_red = MemAlloc(output_width * output_height, 0);
			unsigned char* only_blue = MemAlloc(output_width * output_height, 0);
			unsigned char* only_green = MemAlloc(output_width * output_height, 0);
			for (int i = 0; i < output_height; i++) {
				for (int j = 0; j < output_width; j++) {
					only_red[i * output_width + j] = output_image.data[3 * (i * output_width + j) + 2];
					only_green[i * output_width + j] = output_image.data[3 * (i * output_width + j) + 1];
					only_blue[i * output_width + j] = output_image.data[3 * (i * output_width + j)];
				}
			}
			int idx = 0;
			for (int k = 0; k < 3; k++) {
				for (int i = 0; i < output_height; i++) {
					for (int j = 0; j < output_width; j++) {
						if (k == 0) {
							for_write[idx] = only_red[i * output_width + j];
						}
						else if (k == 1) {
							for_write[idx] = only_blue[i * output_width + j];
						}
						else {
							for_write[idx] = only_green[i * output_width + j];
						}
						idx++;
					}
				}
			...
			for (int i = 0; i < output_height; i++) {
				for (int j = 0; j < output_width; j++) {
					unsigned char red = output_image.data[3 * (i*output_width+j)+2];
					unsigned char green = output_image.data[3 * (i * output_width + j) + 1];
					unsigned char blue = output_image.data[3 * (i * output_width + j)];
					
					unsigned char Y =(unsigned char)(0.299 * red + 0.587 * green + 0.114 * blue);
					unsigned char U = std::max(0, std::min(255, (int)(-0.169 *red-0.331*green+0.499*blue)+128));
					unsigned char V = std::max(0, std::min(255, (int)(0.499 * red - 0.418 * green -0.081 * blue) + 128));
					YUV[3 * (i * output_width + j)]=Y;
					YUV[3 * (i * output_width + j)+1] = U;
					YUV[3 * (i * output_width + j) + 2] = V;
				}
			}

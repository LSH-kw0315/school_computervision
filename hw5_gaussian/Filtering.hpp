...
unsigned char* GrayAverage(unsigned char* img,int rows,int cols,int n) {
	unsigned char* output = MemAlloc((rows - 2 * n) * (cols - 2 * n), 0);
		...
		for (int j = n; j < cols - n; j++) {
			...
			for (int a = 0; a < 2 * n + 1; a++) {
				for (int b = 0; b < 2 * n + 1; b++) {;
					...
					mirror_padding_gray_tmp += img[(i - n + a) * cols + (j - n + b)];			

				}
			}
			output[(i - n) * (cols-2*n) + (j - n)] = mirror_padding_gray_tmp / ((2 * n + 1) * (2 * n + 1));
		}
	}
...
unsigned char* ColorAverage(unsigned char* img, int rows, int cols, int n) {
	unsigned char* output = MemAlloc(3*(rows - 2 * n) * (cols - 2 * n), 0);
	for (int i = n; i < rows - n; i++) {
		for (int j = n; j < cols - n; j++) {
			...
			for (int a = 0; a < 2 * n + 1; a++) {
				for (int b = 0; b < 2 * n + 1; b++) {
					...
					mirror_padding_blue_tmp += img[3*((i - n + a) * cols + (j - n + b))];
					mirror_padding_green_tmp += img[3*((i - n + a) * cols + (j - n + b))+1];
					mirror_padding_red_tmp += img[3*((i - n + a) * cols + (j - n + b))+2];

				}
			}
			output[3*((i - n) * (cols - 2 * n) + (j - n))] = mirror_padding_blue_tmp / ((2 * n + 1) * (2 * n + 1));
			output[3 * ((i - n) * (cols - 2 * n) + (j - n))+1] = mirror_padding_green_tmp / ((2 * n + 1) * (2 * n + 1));
			output[3 * ((i - n) * (cols - 2 * n) + (j - n))+2] = mirror_padding_red_tmp / ((2 * n + 1) * (2 * n + 1));
		}
	}
...
unsigned char* GraySmoothing(unsigned char* img, int rows, int cols,int n) {
	...
	for (int i = n; i < rows - n; i++) {
		for (int j = n; j < cols - n; j++) {
			...
			for (int a = 0; a < 2 * n + 1; a++) {
				for (int b = 0; b < 2 * n + 1; b++) {
					mirror_smooth_gray_tmp += img[((i - n + a) * cols + (j - n + b))] * kernel[((2 * n + 1) - 1 - a) * (2 * n + 1) + (2 * n + 1) - 1 - b];
				

				}
			}

			output[((i - n) * (cols - 2 * n) + (j - n))] = std::max(0, std::min(255, (int)mirror_smooth_gray_tmp));
		}
	}
...
unsigned char* ColorSmoothing(unsigned char* img, int rows, int cols, int n) {
	...
	for (int i = n; i < rows - n; i++) {
		for (int j = n; j < cols - n; j++) {
			...
			for (int a = 0; a < 2 * n + 1; a++) {
				for (int b = 0; b < 2 * n + 1; b++) {
					mirror_smooth_red_tmp += img[3*((i - n + a) * cols + (j - n + b))+2] * kernel[((2 * n + 1) - 1 - a) * (2 * n + 1) + (2 * n + 1) - 1 - b];
					mirror_smooth_green_tmp += img[3 * ((i - n + a) * cols + (j - n + b)) + 1] * kernel[((2 * n + 1) - 1 - a) * (2 * n + 1) + (2 * n + 1) - 1 - b];
					mirror_smooth_blue_tmp += img[3 * ((i - n + a) * cols + (j - n + b))] * kernel[((2 * n + 1) - 1 - a) * (2 * n + 1) + (2 * n + 1) - 1 - b];
				}
			}

			output[3*((i - n) * (cols - 2 * n) + (j - n))+2] = std::max(0, std::min(255, (int)mirror_smooth_red_tmp));
			output[3 * ((i - n) * (cols - 2 * n) + (j - n)) + 1] = std::max(0, std::min(255, (int)mirror_smooth_green_tmp));
			output[3 * ((i - n) * (cols - 2 * n) + (j - n)) ] = std::max(0, std::min(255, (int)mirror_smooth_blue_tmp));
		}
	}

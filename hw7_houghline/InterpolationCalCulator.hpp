...
unsigned char Billinearinterpolation(unsigned char* input, int origin_height, int origin_width, double original_x_point, double original_y_point, int channels,int colorType) {
	...
	double weight_of_p4;
		...
		if (mapping_x_point+1 > origin_width - 1 && mapping_y_point+1 > origin_height - 1) { //맨끝
		...
		else if (mapping_x_point+1> origin_width - 1 && mapping_y_point+1 < origin_height - 1) {//오른쪽 맨끝(x축에서 +1 만큼 떨어진 곳에서 가져올 수 없음)
			p2_x = mapping_x_point - 1;
			p2_y = mapping_y_point;
		...
		else if (mapping_x_point+1< origin_width - 1 && mapping_y_point+1> origin_height - 1) {//맨아래(y축에서 +1 만큼 떨어진 곳에서 가져올 수 없음)
			p2_x = mapping_x_point + 1;
			p2_y = mapping_y_point;
...
unsigned char Bicubicinterpolation(unsigned char* input, int origin_height, int origin_width, double mapping_x_point, double mapping_y_point, int channels,int colorType) {
	...
	double power_of_standard_y_point[4];
		...
		if (mapping_x_point - 1 < 0) { //왼쪽에 아무 픽셀이 없는 경우
			...
			p1_x = p0_x + 1;
			p2_x = p0_x + 2;
			if (mapping_y_point + 1 <= origin_height - 1 && mapping_y_point+2>origin_height-1) {//아래로 칸이 한 칸이 있는 경우
				//이 경우, p1으로 기준점을 옮긴다.
			...
			else if (mapping_y_point+1 > origin_height - 1) {//아래로 칸이 없는 경우
				//이 경우, p1으로 기준점을 옮긴다
			...
			else if (mapping_y_point-1 < 0) {//위로 칸이 없는 경우
				//이 경우, p1으로 기준점을 옮긴다.
		...
		else if (mapping_y_point-1< 0) {//왼쪽 오른쪽에는 문제가 없지만 위로는 칸이 하나도 없음
			//라인을 한 칸 아래로 옮기기만 한다.
		...
		else if (mapping_y_point + 1 <= origin_height - 1 && mapping_y_point + 2 > origin_height - 1) {//아래로 칸이 딱 하나 있을 때
			//라인을 한 칸 위로 옮기기만 한다.
			p0_x = mapping_x_point;
			pM1_x = p0_x - 1;
			p1_x = p0_x + 1;
		...
		else if (mapping_y_point + 1 > origin_height - 1) {//아래로 칸이 없을  때
			//라인을 두 칸 위로 옮기기만 한다.
			p0_x = mapping_x_point;
			pM1_x = p0_x - 1;
			p1_x = p0_x + 1;

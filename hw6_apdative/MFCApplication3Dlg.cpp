...
void CMFCApplication3Dlg::OnBnClickedMainSmoothing()
{
	...
	if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
		...
		output.data = GraySmoothing(GrayScale_Mat.data, GrayScale_Mat.rows, GrayScale_Mat.cols, N);

	}
	else {
		output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
		output.data = ColorSmoothing(original_Image.data, original_Image.rows, original_Image.cols, N);
	}
...
void CMFCApplication3Dlg::OnBnClickedMainAvearge()
{
	...
	if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
		output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC1);
		output.data = GrayAverage(GrayScale_Mat.data, GrayScale_Mat.rows, GrayScale_Mat.cols, N);
		
	}
	else {
		output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
		output.data = ColorAverage(original_Image.data, original_Image.rows, original_Image.cols, N);
	}
	cv::imshow("output", output);
...
void CMFCApplication3Dlg::OnBnClickedMainPiesewise()
{
	...
	if (pieseWise_dlg.DoModal() == IDOK) {
		...
		if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
			output = cv::Mat(GrayScale_Mat.rows,GrayScale_Mat.cols,CV_8UC1);
			output.data = GrayPieseWiseLinear(GrayScale_Mat.data, GrayScale_Mat.rows, GrayScale_Mat.cols, r1, s1, r2, s2);
		}
		else {
			output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
			output.data = ColorPieseWiseLinear(original_Image.data, original_Image.rows, original_Image.cols, r1, s1, r2, s2);
		}
	...
	if (geometricFilter_dlg.DoModal() == IDOK) {
		...
		if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayGeometricMeanFilter(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayGeometricMeanFilter(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
			...
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorGeometricMeanFilter(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorGeometricMeanFilter(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
	...
	if (medianFilter_dlg.DoModal() == IDOK) {
		...
		if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayMedianFilter(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayMedianFilter(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
			...
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorMedianFilter(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorMedianFilter(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
	...
	if (adaptiveMedian_dlg.DoModal() == IDOK) {
		...
		if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayAdaptiveMedianFilter(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayAdaptiveMedianFilter(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
			...
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorAdaptiveMedianFilter(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorAdaptiveMedianFilter(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
	...
	if (meanFilter_dlg.DoModal() == IDOK) {
		...
		if (rawInfo_dlg.GetRawFormat() == ImageFormat::GRAY) {
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayAverage(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
				...
				output = cv::Mat(GrayScale_Mat.rows, GrayScale_Mat.cols, CV_8UC1);
				output.data = GrayAverage(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetGrayPSNR(GrayScale_Mat.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
			...
			if (noiseType == NoiseType::Gaussian && GaussianNoiseImage.empty()) {
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorAverage(GaussianNoiseImage.data, GaussianNoiseImage.rows, GaussianNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
				...
				output = cv::Mat(original_Image.rows, original_Image.cols, CV_8UC3);
				output.data = ColorAverage(SaltAndPepperNoiseImage.data, SaltAndPepperNoiseImage.rows, SaltAndPepperNoiseImage.cols, kernel_size);
				psnr = GetColorPSNR(original_Image.data, output.data, GrayScale_Mat.rows, GrayScale_Mat.cols);
			}
